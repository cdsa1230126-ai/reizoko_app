...
class _ReizokoAppState extends State<ReizokoApp> {
  ...
  Future<void> _generateRecipe() async {
    ...
    final prompt =
        "あなたは$charNameです。冷蔵庫にある「$ingredients」を使って、気分が「$_aiMood」にぴったりの料理レシピを1つ提案してください。$charNameらしい口調で、材料と手順を簡潔に教えてください。";
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _aiResult = "通信エラーじゃ。APIキーを確認しておくれ。";
        });
      }
    } catch (e) {
      setState(() {
        _aiResult = "エラーが発生したわい。接続を確認してな。";
      });
    } finally {
      setState(() {
        _isAiLoading = false;
            ...
            children: [
              ...
              TextField(
                controller: TextEditingController(text: _apiKey)
                  ..selection = TextSelection.fromPosition(TextPosition(offset: _apiKey.length)),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Expanded(
                child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [_label("3. 単位", textColor), _drop(units, _unit, (v) => setState(() => _unit = v!))])),
            const SizedBox(width: 15),
            Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              InkWell(
                  ...
                  child: Container(
                      ...
                      child: Text("${_date.year}/${_date.month}/${_date.day}", style: TextStyle(color: textColor))))
            ])),
          ]),
          const SizedBox(height: 20),
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            _label("5. 個数", textColor),
            Row(children: [
              Text("★ お気に入り", style: TextStyle(color: textColor, fontSize: 12)),
              Switch(value: _isFav, activeColor: Colors.yellowAccent, onChanged: (v) => setState(() => _isFav = v))
            ])
          ]),
          Row(children: [
            Expanded(
                child: _drop(List.generate(20, (i) => (i + 1).toString()), _count.toInt().toString(),
                    (v) => setState(() => _count = double.parse(v!)))),
            IconButton(
          ...
          if (_cat == "飲み物") ...[
            const SizedBox(height: 20),
            _label("🥤 容量設定", textColor),
            Row(children: [
              Expanded(
                  child: TextField(
                      keyboardType: TextInputType.number,
                      style: TextStyle(color: textColor),
                      decoration: const InputDecoration(filled: true, fillColor: Colors.black26, hintText: "500"),
                      onChanged: (v) => _vol = double.tryParse(v) ?? 500)),
            ...
            onPressed: () {
              ...
              setState(() {
                inventory.add({
                  ...
                  "step": step,
                  "isFavorite": _isFav,
                  "vol": _cat == "飲み物" ? _vol : null,
                  "vUnit": _cat == "飲み物" ? _vUnit : null,
                });
                _tabIdx = 0;
              });
      ...
      itemBuilder: (context, i) {
        ...
        return Card(
          ...
          child: ListTile(
            leading: InkWell(
                onTap: () {
                  setState(() => inventory[i]["isFavorite"] = !isF);
                  _save();
                },
                child: Stack(
                    alignment: Alignment.bottomRight,
                    children: [Text(item["icon"], style: const TextStyle(fontSize: 30)), Icon(isF ? Icons.star : Icons.star_border, color: Colors.yellowAccent, size: 18)])),
                ...
                onPressed: () {
                  setState(() {
                    if (item["count"] > item["step"]) {
                      ...
                      if (sIdx != -1) {
                        shoppingList[sIdx]["count"] += 1.0;
                      } else {
                        shoppingList.add(Map.from(item)..["count"] = 1.0);
                      }
      ...
      itemBuilder: (context, i) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            title: Text("${(item["isFavorite"] ?? false) ? '★ ' : ''}${item["name"]} (×${item["count"]})",
    ...
    if (_apiKey.isEmpty) {
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          ElevatedButton(
              style: ElevatedButton.styleFrom(
                  padding: const EdgeInsets.symmetric(horizontal: 30, vertical: 15), backgroundColor: Colors.cyanAccent, foregroundColor: Colors.black),
              onPressed: _showApiKeySetting,
              child: const Text("APIキーを設定する", style: TextStyle(fontWeight: FontWeight.bold))),
        ]),
      );
    }
    ...
    return Column(
      children: [
        Padding(
          ...
          child: Column(
            children: [
              Text("今の気分を選んでおくれ", style: TextStyle(color: textColor, fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 20),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: moods.map((m) {
                  final isSelected = _aiMood == m;
                  return Expanded(
                    child: Padding(
                      ...
                      child: SizedBox(
                        ...
                        child: ElevatedButton(
                          style: ElevatedButton.styleFrom(
                            backgroundColor: isSelected ? Colors.yellowAccent : Colors.black45,
                            foregroundColor: isSelected ? Colors.black : Colors.white,
                            elevation: isSelected ? 8 : 0,
                            shape: RoundedRectangleBorder(
                              borderRadius: BorderRadius.circular(12),
                              side: BorderSide(color: isSelected ? Colors.yellowAccent : Colors.white24),
                            ),
                            padding: EdgeInsets.zero,
                          ),
                          onPressed: () => setState(() => _aiMood = m),
                          child: Text(m, style: TextStyle(fontSize: 15, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal)),
                        ),
                      ),
                    ),
                  );
              ...
              ElevatedButton.icon(
                ...
                label: Text(
                  _isAiLoading ? "考え中じゃ..." : "${chars[modeIndex]["n"]}にレシピを聞く",
                    ...
                    : () {
                        if (inventory.isEmpty) {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                            content: Text("冷蔵庫が空じゃ！食材を登録してから聞いておくれ。"),
                  ...
                  ? const Center(
                      child: Column(
                        children: [
                          ...
                          SizedBox(height: 20),
                          Text("美味しい献立を考えておるぞ...", style: TextStyle(color: Colors.white70)),
                        ],
                      ),
                    )
                  : Text(
                      _aiResult.isEmpty ? "上のボタンを押すと、${chars[modeIndex]["n"]}が秘伝のレシピを教えてくれるぞ。" : _aiResult,
  ...
  Widget _label(String t, Color c) =>
      Padding(padding: const EdgeInsets.only(bottom: 8), child: Text(t, style: TextStyle(color: c, fontWeight: FontWeight.bold, fontSize: 14)));
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          ...
          ...List.generate(
              3,
              (i) => RadioListTile(
                  title: Text("${chars[i]["i"]} ${chars[i]["n"]}", style: const TextStyle(color: Colors.white)),
                  value: i,
                  groupValue: modeIndex,
                  onChanged: (v) {
                    setState(() => modeIndex = v!);
                    _save();
                    Navigator.pop(ctx);
                  })),
          ...
          const SizedBox(height: 15),
          ElevatedButton(
              child: const Text("自由な色を選ぶ"),
              onPressed: () async {
                final res = await js.context.callMethod('eval', [
                  "new Promise((r) => { const i = document.createElement('input'); i.type = 'color'; i.onchange = () => r(i.value); i.click(); });"
                ]);
                if (!mounted) return;
                if (res != null) {
                  setState(() => customColor = Color(int.parse("FF${res.toString().replaceFirst('#', '')}", radix: 16)));
                  _save();
                  Navigator.pop(ctx);
                }
              })
