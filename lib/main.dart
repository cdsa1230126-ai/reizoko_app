...
class _ReizokoAppState extends State<ReizokoApp> {
  ...
  void _moveToShopping(int i) {
    ...
    _speak("${item["name"]}を使い切ったぞ。リストに追加した。");
  ...
  Widget _itemTile(dynamic item, int i, bool isInv) {
    ...
    return Card(
      color: cardColor, 
      elevation: isInv ? 1 : 0, // 買い物タブの影を消して灰色の四角を防止
      child: ListTile(
  ...
  Widget _itemCard(dynamic item, int i, bool isInv) {
    return Card(
      color: isInv ? Colors.black45 : Colors.transparent, 
      elevation: isInv ? 1 : 0,
  ...
  Future<void> _generateRecipe() async {
    ...
    try {
      final res = await http.post(Uri.parse("https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=$_apiKey"), headers: {'Content-Type': 'application/json'}, body: jsonEncode({"contents": [{"parts": [{"text": "あなたは${chars[modeIndex]["n"]}です。語尾は${chars[modeIndex]["s"]}を使って。${inventory.map((e)=>e['name']).join(',')}で${_aiMood}なレシピを作って。"}]}]}));
